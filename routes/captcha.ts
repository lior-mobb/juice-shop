...
function captchas () {
  return async (req: Request, res: Response) => {
    ...
    const secondTerm = Math.floor(((crypto.getRandomValues(new Uint32Array(1))[0] / 2 ** 32) * 10) + 1)
    const thirdTerm = Math.floor((Math.random() * 10) + 1)

    const firstOperator = operators[Math.floor(((crypto.getRandomValues(new Uint32Array(1))[0] / 2 ** 32) * 3))]
