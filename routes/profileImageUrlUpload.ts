        ...
        const imageRequest = request
          ...
          .on('response', function (res: Response) {
            if (res.statusCode === 200) {
              const safeInput = path.basename(String(url.split('.').slice(-1)[0] || '').replace('\0', '').replace(/^(\.\.(\/|\\$))+/, ''))
              const ext = ['jpg', 'jpeg', 'png', 'svg', 'gif'].includes(url.split('.').slice(-1)[0].toLowerCase()) ? safeInput.toLowerCase() : 'jpg'
